...
def page():
    ...
    **Post-processing**: AntiSMASH outputs can be processed with custom scripts to extract JSON data for further analysis, see [02_pipeline](https://github.com/AmauryMaros/BGC_antiSMASH).
    ...
    st.subheader("Run BiG-SCAPE", divider="grey")
    ...
    # Activate conda env
    source /usr/local/packages/miniconda3/etc/profile.d/conda.sh
    conda activate path_to_bigscape_env

    # Run bigscape
    # input_dir is a directory with all .gbk files generated by antiSMASH
    bigscape cluster -c 6 -i input_dir/ -o output_dir -p pfam/Pfam-A.hmm --mix
            """)
